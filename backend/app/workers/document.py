...
def _find_agreement_date(schema: dict, lender_name: str) -> str | None:
    ...
    for acc in accounts:
        ...
        if acc_lender == name_lower:
            return acc.get("open_date") or acc.get("agreement_date")
...
def generate_documents(self, job_id: int):
    ...
    try:
        ...
        for result in lender_results:
            if result.traffic_light not in ("GREEN", "AMBER"):
                continue
            if is_blocked(result.lender_name):
                continue
        ...
        if job.batch_id:
            ...
            if batch:
                batch.assessments_generated += 1
                batch.locs_generated += loc_count
                batch.processed += 1
